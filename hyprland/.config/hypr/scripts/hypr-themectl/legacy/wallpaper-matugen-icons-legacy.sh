...
# =============================================================================
# One-Click Wallpaper + Matugen + Papirus Folders + System Icons + ReGreet
#
# Minimal CLI:
#   - default: random wallpaper
#   - --set <path|filename>
#   - --reapply
#   - --debug
#
# Always-on behavior:
#   - Runs matugen (if available)
#   - Detects grayscale wallpapers and uses neutral folder colors (if ImageMagick)
#   - Applies folder icons (Papirus + Catppuccin variants)
#   - Updates GTK/Qt configs to use the derived icon theme
#   - Installs derived icon theme system-wide for ReGreet
#   - Updates ReGreet:
#       - /etc/greetd/regreet.toml: icon_theme_name
#       - /etc/greetd/regreet.css: includes Matugen vars + styles ReGreet widgets
#       - background (blur if possible)
#       - OPTIONAL: patch /etc/greetd/config.toml to add '--style /etc/greetd/regreet.css'
#   - Reloads desktop components
#   - Lock: auto-breaks if busy, then retries once
#
# Notes:
#   - ReGreet supports a global stylesheet called regreet.css (and --style).
...
Environment:
  WALLDIR                   Wallpaper directory (default: ~/Pictures/walls)
  MATUGEN_MODE              dark|light|amoled (default: dark)
  REGREET_BG_BLUR_RADIUS    ImageMagick blur (default: 20x8)
EOF
    exit 0
    ;;
  *)
...
preflight() {
  local missing=()
  for c in "$CLI" "$DAEMON" hyprctl jq rsync git curl flock fuser; do
    is_cmd "$c" || missing+=("$c")
  done
...
write_regreet_style_css() {
  ...
  # Pull colors ONLY from Matugen->Starship palette (stable hex values)
  ...
  # Make "destructive" follow the same accent as Login (suggested-action)
...
PY
  ...
  # UI tint for pickers/entries = same hue as Login (accent)
  ...
  # Darker password area (still same hue)
...
/* Make destructive follow accent too (so reboot/poweroff match Login) */
...
/* Remove divider line above bottom buttons (GtkActionBar + bottom container)
   GtkActionBar node structure: actionbar -> revealer -> box -> box.start/center/end :contentReference[oaicite:1]{index=1} */
actionbar,
actionbar > revealer,
...
/* In case a separator exists inside that bottom area */
actionbar separator,
...
dropdown button:disabled,
menubutton > button.toggle:disabled,
combobox > box.linked > button.combo:disabled,
combobox > box.linked > entry.combo:disabled,
row.combo:disabled {
  opacity: 1;
}
...
/* Login (and anything suggested-action) */
...
/* Reboot/PowerOff often use destructive-action: force same as Login */
...
# Patch greetd config to add explicit --style (idempotent)
...
apply_regreet_background() {
  ...
  # IMPORTANT: include quality/format in the cache key so background regenerates
  state_key="${wall_hash}_${REGREET_BG_BLUR_RADIUS}_${REGREET_BG_FIT}_${target_res}_${REGREET_BG_FORMAT}_${REGREET_BG_QUALITY}_${REGREET_BG_SAMPLING}"
  prev_state="$(read_state "$STATE_REGREET_BG_FILE")"
  if [[ "$prev_state" == "$state_key" ]]; then
  ...
  if im_available; then
    ...
    if [[ "$ext" == "png" ]]; then
      # PNG: no JPEG artifacts, typically looks the cleanest for blur
      if "$cmd" "$wall" \
        -filter Lanczos \
        -resize "${target_res}^" \
        ...
        log "Warning: ImageMagick failed, using original wallpaper"
        root_exec install -m 644 "$wall" "$dest"
      fi
    else
      # JPEG: push quality + disable chroma subsampling as much as possible
      # -quality and -sampling-factor are ImageMagick options for JPEG encoders. :contentReference[oaicite:2]{index=2}
      # dct-method=float can reduce some artifacts/banding in some cases. :contentReference[oaicite:3]{index=3}
      if "$cmd" "$wall" \
        -filter Lanczos \
        -resize "${target_res}^" \
        -gravity center -extent "$target_res" \
    ...
    log "ImageMagick not available, using original wallpaper"
    root_exec install -m 644 "$wall" "$dest"
  fi
